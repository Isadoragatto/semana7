...
**B) O código avalia a expressão booleana, imprime o resultado `true`, calcula a soma dos números de 1 a 5 e imprime o resultado no console**       ESSA É A RESPOSTA CERTA
...
**B) A saída de calcularOrcamento() seria: 'Seu saldo é negativo de -1050.' e a de calcularOrcamento2() seria: 'Seu saldo é negativo de -100.'** ESSA É A RESPOSTA CORRTEA 
...
**B) Preload -> Create -> Update** ESSA É A RESPOSTA CERTA
...
**B) Simular interações físicas realistas, como colisões e movimentos, em jogos 2D.** ESSA É A RESPOSTA CERTA
...
variavel idade = insira("insira sua idade")
se a idade for menor que 16
imprime "não pode votar"
se a idade for maior ou igaul a 16 E menor que 18
...
Classe Retangulo estendendo FormaGeometrica{
 ...
 Método Construtor(altura, base, cor):
        Chamar Construtor de FormaGeometrica com parâmetro cor
        Definir o valor do atributo altura com o valor passado como parâmetro
        Definir o valor do atributo base com o valor passado como parâmetro


método CalcularAreaRetangulo()
        area <- base * altura
        imprime("A área do retângulo é igual a " + area)
}
...
Classe Circulo estendendo FormaGeometrica {
 ...
 Método CalcularArea():
        area <- 3.14 * raio * raio
        imprime("Este círculo tem área de " + area)
}
...
variavel tempo = 0,
variavel distancia = 0

enquanto distancia for < distanciaTotal e tempo for <= tempoMaximo:
    calcula a velocidade atual usando: velocidade = velocidadeInicial + aceleracao * tempo

    se velocidade for > velocidadeMaxima,
    então velocidade = velocidadeMaxima

    atualiza a distancia percorrida: distancia = distancia + velocidade
    incrementa o tempo em 1: tempo = tempo + 1

se distancia for >= distanciaTotal:
    imprime "O carro da Tesla conseguiu terminar a corrida em " + tempo + " minutos"
senão se tempo for > tempoMaximo:
    imprime "O carro da Tesla não conseguiu completar a corrida no tempo máximo"
...
Se tamanho(matrizA[0]) for ≠ tamanho(matrizB) então:
    ...
    Senão:
        ...
        Loop para percorrer cada elemento da matrizResultado e calcular o valor
